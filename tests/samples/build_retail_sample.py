...
TABLES = [
    {"name": "Sales", "columns": [col(c) for c in ("OrderKey", "DateKey", "ProductKey", "CustomerKey", "StoreKey",
     ...
     "measures": [
         ...
         {"name": "Gross Margin", "expression": "[Revenue]-[Total Cost]"},
         {"name": "Gross Margin %", "expression": "DIVIDE([Gross Margin],[Revenue])"},
         {"name": "Units", "expression": "SUM(Sales[Quantity])"},
         {"name": "Revenue LY", "expression": "CALCULATE([Revenue], SAMEPERIODLASTYEAR('Date'[Date]))"},
         {"name": "Revenue YoY %", "expression": "DIVIDE([Revenue]-[Revenue LY],[Revenue LY])"},
         {"name": "Revenue YTD", "expression": "TOTALYTD([Revenue],'Date'[Date])"},
